...
namespace PharmacyContracts.Modules.Claims.Application.Interfaces
{
    public interface IClaimRepository : IGenericRepository<Claim>
    {
        ...
        Task<decimal> GetTotalClaimedAsync(Guid pharmacyId, string? companyName, int? month, int? year, CancellationToken cancellationToken = default);
        Task<List<string>> GetDistinctCompanyNamesAsync(Guid pharmacyId, int? month, int? year, CancellationToken cancellationToken = default);
